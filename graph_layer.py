...
class G_Layer:
    def __init__(self):
        ...
        self.parents = []#index parents directs ou indirects          ******************************Appliquer la fonction d'update!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        self.nb_pool_parent_tot = 0#max nb pr parents total
        self.nb_pool_enfant_tot = 0
        self.nb_deconv_parent_tot = 0
        self.nb_deconv_enfant_tot = 0
    ...
    def update_max_branches(self,type_brch='parents'):
        ...
        for index,p in enumerate(liste_concernee):
            if type_brch == 'parent':
                Lnb_pool[index] = graph_controleur.G_Controleur.couches_graph[index].nb_pool_parent_tot+self.couche_pool
                Lnb_deconv[index] = graph_controleur.G_Controleur.couches_graph[index].nb_deconv_parent_tot+self.couche_deconv
            else:
                Lnb_pool[index] = graph_controleur.G_Controleur.couches_graph[index].nb_pool_enfant_tot
                Lnb_deconv[index] = graph_controleur.G_Controleur.couches_graph[index].nb_deconv_enfant_tot
        if type_brch == 'parent':
...
class G_Conv(G_Layer):
    ...
    def __init__(self):
        ...
        self.filters = graph_controleur.G_Controleur.hp.Choice('filtre_conv_index_%d'%(graph_controleur.G_Controleur.couche_id),possibilites_filtres,default=1)
        self.kernel = graph_controleur.G_Controleur.hp.Choice('kernel_conv_index_%d'%(graph_controleur.G_Controleur.couche_id),[2,3],default=2)
        self.activ = graph_controleur.G_Controleur.hp.Choice("activation_conv_index_%d"%(graph_controleur.G_Controleur.couche_id),['linear','relu','elu','selu','tanh'],default='relu')
        self.couche = Conv2D(filters=self.filters,
                             kernel_size=self.kernel,
                             padding='SAME',
                             activation=self.activ,
                             name='Convolution_id_%d_k%d_f%d_activ_%s'%(graph_controleur.G_Controleur.couche_id,self.filters,self.kernel,self.activ))
        graph_controleur.G_Controleur.add_couche(self)
        graph_controleur.G_Controleur.graph.node(str(self.couche_id),shape='record',label="{Conv %d|{{Noyau|%d}|{Filtres|%d}|{Activation|%s}}}"%(self.couche_id,self.kernel,self.filters,self.activ))

from tensorflow.keras.layers import MaxPooling2D,AveragePooling2D
class G_Pool(G_Layer):
    def __init__(self):
        super(G_Pool,self).__init__()
        self.type_pool = graph_controleur.G_Controleur.hp.Choice('pool_index_%d'%(graph_controleur.G_Controleur.couche_id),['avg','max'],default='max')
        self.couche_pool = 1
        ...
        graph_controleur.G_Controleur.add_couche(self)
        graph_controleur.G_Controleur.graph.node(str(self.couche_id),shape='record',label="{%spooling %d}"%("Max" if self.type_pool == 'max' else 'Average',self.couche_id))
...
class G_Deconv(G_Layer):
    def __init__(self):
        ...
        self.nb_deconv_parent_tot = 1
        graph_controleur.G_Controleur.add_couche(self)
        graph_controleur.G_Controleur.graph.node(str(self.couche_id),shape='record',label="{Deconv %d|{{Filtres|%d}|{Activation|%s}}}"%(self.couche_id,self.filters,self.activ))
