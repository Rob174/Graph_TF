...
class G_Controleur:
    ...
    def lier(couche_id_1,couche_id_2,forcer=False):
        lien = False
        #Vérifications
        verifications_ok = couche_id_1 != couche_id_2#Ce n'est pas la couche courante
        ...
        dim_crt =                   -G_Controleur.couches_graph[couche_id_1].nb_pool_parent_tot+     G_Controleur.couches_graph[couche_id_1].nb_deconv_parent_tot
        ...
        dim_nd_sel =                          -G_Controleur.couches_graph[couche_id_2].nb_pool_parent_tot+   G_Controleur.couches_graph[couche_id_2].nb_deconv_parent_tot   +G_Controleur.couches_graph[couche_id_1].couche_pool  -G_Controleur.couches_graph[couche_id_2].couche_deconv #on exclut la couche courante
        ...
        if diff_taille != 0 and forcer == True:
            ...
            if diff_taille > 0:
                for i in range(diff_taille):
                    graph_layer.G_Pool()
                    G_Controleur.couches_graph[-1].invisible_adapt = True
                    G_Controleur.lier(couche_adapt.couche_id.couche_id,G_Controleur.couches_graph[-1].couche_id,forcer=True)
                    couche_adapt = G_Controleur.couches_graph[-1]
            else:
                for i in range(-diff_taille):
                    graph_layer.G_Deconv()
                    G_Controleur.couches_graph[-1].invisible_adapt = True
                    G_Controleur.lier(couche_adapt.couche_id,G_Controleur.couches_graph[-1].couche_id,forcer=True)
                    couche_adapt = G_Controleur.couches_graph[-1]
            G_Controleur.lier(G_Controleur.couches_graph[-1].couche_id,G_Controleur.couches_graph[couche_id_2].couche_id,forcer=True)
            return 
        if lien==True:
